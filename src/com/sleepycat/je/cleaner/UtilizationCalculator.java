...
public class UtilizationCalculator {
    ...
    /* Min/max correction factors used to prohibit unreasonable values. */
    private static final float MIN_CORRECTION_FACTOR = 0.01f;
    private static final float MAX_CORRECTION_FACTOR = 100f;
    ...
    UtilizationCalculator(EnvironmentImpl env, Cleaner cleaner) {
        ...
        configAdjustUtilization = configManager.getBoolean
            (EnvironmentParams.CLEANER_ADJUST_UTILIZATION);
    ...
    /**
     * Returns EnvironmentConfig.CLEANER_ADJUST_UTILIZATION value.
     */
    public boolean isAdjustUtilizationEnabled() {
        return configAdjustUtilization;
    }

    /**
     * Returns the obsolete size for a FileSummary, and applies the LN size
     * correction factor if adjustment is configured.  This method should be
     * used rather than calling FileSummary.getObsoleteSize directly, so that
     * the configuration setting can be honored.
     */
    public int getObsoleteSize(FileSummary summary) {
        if (configAdjustUtilization) {
            return summary.getObsoleteSize(lnSizeCorrectionFactor);
        }
     ...
     */
    synchronized Long getBestFile(SortedMap<Long, FileSummary> fileSummaryMap,
        ...
        if (logger.isLoggable(logLevel)) {
            final String fileChosenString = (fileChosen != null) ?
                ...
                "";
            final String correctionString = configAdjustUtilization ?
                ( " lnSizeCorrectionFactor: " + lnSizeCorrectionFactor) :
                " (adjustment disabled)";
            LoggerUtils.logMsg
                (logger, env, logLevel,
                 loggingMsg + fileChosenString + correctionString +
     ...
     *
     * Note that these values represent only the OBSOLETE uncounted amounts,
     * while the estimate further above uses ALL uncounted amounts.
     *
     ...
     *
     * @return whether the adjustment was valid, or no adjustment was needed.
     * False is returned if the adjustment was discarded because it is invalid.
     */
    synchronized boolean adjustUtilization(long fileNum,
                                           long endFileNum,
        ...
        /*
         * If estimated and true obsolete counts are unequal then comparing the
         * sizes would lead to incorrect conclusions, since the true obsolete
         * size that corresponds to the estimated obsolete count is unknown.
         * The obsolete counts might be unequal if estimated values are
         * double-counted by recovery, for example, or if the file's
         * utilization changes in between the estimate and the cleaning/probe.
         * [#22814]
         */
        if (estimatedFileSummary.obsoleteLNCount !=
            trueFileSummary.obsoleteLNCount) {
            return false;
        }
         ...
         * The uncountedSize and uncountedCount values represent only the
         * OBSOLETE uncounted amounts, while estUncountedSize and
         * estUncountedCount represented ALL uncounted amounts.
         */
        ...
        if (uncountedSize > 0 && uncountedCount > 0) {
            ...
            /*
             * Calc this file's correction factor and apply this adjustment
             * only when it is a reasonable value.
             */
            trueAvgSize = uncountedSize / ((float) uncountedCount);
            correctionFactor = trueAvgSize / estimatedAvgSize;

            if (correctionFactor < MIN_CORRECTION_FACTOR ||
                correctionFactor > MAX_CORRECTION_FACTOR) {
                return false;
            }
